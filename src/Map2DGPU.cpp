...
bool Map2DGPU::Map2DGPUEle::updateTextureCPU() {
    ...
    if (texName == 0)  // create texture
    {
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, tmp.cols, tmp.rows, 0, GL_BGRA, GL_FLOAT, tmp.data);
...
bool Map2DGPU::Map2DGPUEle::updateTextureGPU() {
    ...
    if (texName == 0)  // create texture
    {
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, ELE_PIXELS, ELE_PIXELS, 0, GL_BGRA, GL_FLOAT, NULL);
...
bool Map2DGPU::feed(cv::Mat img, cv::Mat sem, const pi::SE3d& pose) {
    if (!_valid)
        return false;
...
bool Map2DGPU::renderFrame(const CameraFrame& frame) {
    ...
    int cenY = (translation.y - d->min().y) * d->lengthPixelInv();
    {
        ...
        bool success = renderFramesCaller(cudaFrame, ELE_PIXELS, ELE_PIXELS, out_datas, freshs, invs, centers, wh);
...
bool Map2DGPU::save(const std::string& filename) {
    ...
    for (int x = minInt.x; x < d->w(); x++)
        for (int y = minInt.y; y < d->h(); y++) {
            ...
            cudaMemcpy(tmp.data, ele->img, ELE_PIXELS * ELE_PIXELS * sizeof(float4), cudaMemcpyDeviceToHost);
            tmp.copyTo(
                    result(cv::Rect(ELE_PIXELS * (x - minInt.x), ELE_PIXELS * (y - minInt.y), ELE_PIXELS, ELE_PIXELS)));
        }
