...
class Map2DRender : public Map2D, public pi::Thread {
    ...
    struct Map2DRenderData  // change when spread and prepare
    {
        Map2DRenderData()
            : _w(0),
              _h(0) {}
        Map2DRenderData(double eleSize_, double lengthPixel_, pi::Point3d max_, pi::Point3d min_, int w_, int h_,
                const std::vector<SPtr<Map2DRenderEle> >& d_)
            : _eleSize(eleSize_),
