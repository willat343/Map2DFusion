 ...
 */
bool MultiBandMap2DCPUSem::MultiBandMap2DCPUSemEle::normalizeUsingWeightMap(const cv::Mat &weight, cv::Mat &src) {
 ...
 * @return false Wrong matirx type
 */
bool MultiBandMap2DCPUSem::MultiBandMap2DCPUSemEle::mulWeightMap(const cv::Mat &weight, cv::Mat &src) {
    // Check type of input matrixes
    if (!(src.type() == CV_32FC3 && weight.type() == CV_32FC1)) {
        return false;
    }
                ...
                for (int i = 0; i < pyr_laplace.size(); i++) {
                    ...
                    for (int y = 0; y < 3; y++) {
                        for (int x = 0; x < 3; x++) {
                            ...
                            const SPtr<MultiBandMap2DCPUSemEle> &ele = neighbors[3 * y + x];
...
MultiBandMap2DCPUSem::MultiBandMap2DCPUSemData::MultiBandMap2DCPUSemData(double eleSize_, double lengthPixel_, pi::Point3d max_,
    ...
    : _eleSize(eleSize_),
      ...
      _w(w_),
      _h(h_),
      _data(d_)  // Vector of MultiBandMap2DCPUSemEle
{
    _gpsOrigin = svar.get_var("GPS.Origin", _gpsOrigin);  // Set GPS origin
}

bool MultiBandMap2DCPUSem::MultiBandMap2DCPUSemData::prepare(SPtr<MultiBandMap2DCPUSemPrepare> prepared) {
...
/**
 * @brief Constructor for MultiBandMap2DCPUSem
 *
...
bool MultiBandMap2DCPUSem::prepare(const pi::SE3d &plane, const PinHoleParameters &camera,
        const std::deque<std::pair<cv::Mat, pi::SE3d>> &frames) {
    // insert frames
    SPtr<MultiBandMap2DCPUSemPrepare> p(new MultiBandMap2DCPUSemPrepare);
    SPtr<MultiBandMap2DCPUSemData> d(new MultiBandMap2DCPUSemData);
...
bool MultiBandMap2DCPUSem::feed(cv::Mat img, const pi::SE3d &pose) {
    ...
    // Get p and d
    SPtr<MultiBandMap2DCPUSemPrepare> p;
    SPtr<MultiBandMap2DCPUSemData> d;
    {
...
bool MultiBandMap2DCPUSem::renderFrame(const std::pair<cv::Mat, pi::SE3d> &frame) {
    ...
    if (frame.first.cols != p->_camera.w || frame.first.rows != p->_camera.h || frame.first.type() != CV_8UC3) {
        cerr << "MultiBandMap2DCPUSem::renderFrame: "
    ...
    if (xminInt < 0 || yminInt < 0 || xmaxInt > d->w() || ymaxInt > d->h() || xminInt >= xmaxInt ||
            yminInt >= ymaxInt) {
        cerr << "MultiBandMap2DCPUSem::renderFrame:should never happen!\n";
        return false;
    }
    ...
    // Convert image to 3-channel float (if MultiBandMap2DCPUSem.ForceFloat == 0) or 3-channel 2-byte signed int.
    cv::Mat img_src;
    if (svar.GetInt("MultiBandMap2DCPUSem.ForceFloat", 0)) {
    ...
    cv::Mat sem_warped((ymaxInt - yminInt) * ELE_PIXELS, (xmaxInt - xminInt) * ELE_PIXELS, sem.type());
    ...
    std::vector<SPtr<MultiBandMap2DCPUSemEle>> dataCopy = d->data();
    for (int x = xminInt; x < xmaxInt; x++) {
        for (int y = yminInt; y < ymaxInt; y++) {
            // Get the cell/element patch
            SPtr<MultiBandMap2DCPUSemEle> ele = dataCopy[y * d->w() + x];
            if (!ele.get()) {
                ele = d->ele(y * d->w() + x);
            }
    ...
    pi::timer.leave("MultiBandMap2DCPUSem::Apply");
 ...
 * @return false
 */
bool MultiBandMap2DCPUSem::spreadMap(double xmin, double ymin, double xmax, double ymax) {
    // Start spreadMap timer
    pi::timer.enter("MultiBandMap2DCPUSem::spreadMap");
    ...
    std::vector<SPtr<MultiBandMap2DCPUSemEle>> dataOld = d->data();
    std::vector<SPtr<MultiBandMap2DCPUSemEle>> dataCopy;
    dataCopy.resize(w * h);
    ...
    {
        pi::WriteMutex lock(mutex);
        data = SPtr<MultiBandMap2DCPUSemData>(new MultiBandMap2DCPUSemData(d->eleSize(), d->lengthPixel(),
                pi::Point3d(max.x, max.y, d->max().z), pi::Point3d(min.x, min.y, d->min().z), w, h, dataCopy));
    }

    // Stop spreadMap timer
    pi::timer.leave("MultiBandMap2DCPUSem::spreadMap");
    return true;
}
 ...
 */
bool MultiBandMap2DCPUSem::getFrame(std::pair<cv::Mat, pi::SE3d> &frame) {
    // Take Mutexes
    pi::ReadMutex lock(mutex);
    ...
    while (!shouldStop()) {
        ...
        if (_valid) {
            ...
            if (getFrame(frame)) {
                ...
                pi::timer.enter("MultiBandMap2DCPUSem::renderFrame");
                // Render the newly extracted frame from the queue
                renderFrame(frame);
                // Stop renderFrame timer
                pi::timer.leave("MultiBandMap2DCPUSem::renderFrame");
            }
        }
        sleep(10);
    }
 ...
 *
 */
void MultiBandMap2DCPUSem::draw() {
    // Check if MultiBandMap2DCPUSem was prepared
    if (!_valid)
        return;
    ...
    for (int x = 0; x < wCopy; x++) {
        for (int y = 0; y < hCopy; y++) {
            ...
            // Get Ele from vector
            SPtr<MultiBandMap2DCPUSemEle> ele = dataCopy[idxData];
...

            {
                ...
                if (ele->Ischanged) {
                    ...
                    pi::timer.leave("MultiBandMap2DCPUSem::updateTexture");
                    ...
                    if (updated && !inborder && svar.GetInt("Fuse2Google")) {
                        ...
                        pi::timer.enter("MultiBandMap2DCPUSem::fuseGoogle");
                        stringstream cmd;
                        ...
                        pi::timer.leave("MultiBandMap2DCPUSem::fuseGoogle");
                    }
                }
            }
...
bool MultiBandMap2DCPUSem::save(const std::string &filename) {
    ...
    for (int x = 0; x < d->w(); x++) {
        for (int y = 0; y < d->h(); y++) {
            SPtr<MultiBandMap2DCPUSemEle> ele = d->data()[x + y * d->w()];
            if (!ele.get()) {
                continue;
            }
