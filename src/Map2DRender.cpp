...
bool mulWeightMap(const cv::Mat& weight, cv::Mat& src) {
    if (!(src.type() == CV_16SC3 && weight.type() == CV_32FC1))
        return false;
...
public:
    ...
    void prepare(cv::Rect dst_roi) {
        ...
        for (int i = 1; i <= num_bands_; ++i) {
            dst_pyr_laplace_[i].create((dst_pyr_laplace_[i - 1].rows + 1) / 2, (dst_pyr_laplace_[i - 1].cols + 1) / 2,
                    CV_16SC3);
    ...
    void feed(const cv::Mat& img, const cv::Mat& mask, cv::Point tl) {
        ...
        Mat img_with_border;
        copyMakeBorder(img, img_with_border, top, bottom, left, right, BORDER_REFLECT);
        ...
        for (int i = 0; i < num_bands_; ++i) pyrDown(weight_pyr_gauss[i], weight_pyr_gauss[i + 1]);
        ...
        if (svar.GetInt("Map2DRender.ShowPyrLaplace", 1)) {
            ...
            for (int i = 0; i < src_pyr_laplace.size(); i++) {
                ...
                {
                    cv::Mat result;
                    pyr_laplaceClone[i].convertTo(result, CV_8U);
    ...
    void blend(cv::Mat& dst, cv::Mat& dst_mask) {
        ...
        if (should_normalize)
            for (int i = 0; i <= num_bands_; ++i) normalizeUsingWeightMap(dst_band_weights_[i], dst_pyr_laplace_[i]);
...
bool Map2DRender::feed(cv::Mat img, cv::Mat sem, const pi::SE3d& pose) {
    if (!_valid)
        return false;
...
bool Map2DRender::renderFrames(std::deque<CameraFrame>& frames) {
    ...
    for (std::deque<CameraFrame>::iterator it = frames.begin(); it < frames.end(); it++, idx++) {
        ...
        for (int i = 0; i < planePts.size(); i++) {
            if (planePts[i].x < curMin.x)
                curMin.x = planePts[i].x;
            if (planePts[i].y < curMin.y)
                curMin.y = planePts[i].y;
            if (planePts[i].x > curMax.x)
                curMax.x = planePts[i].x;
            if (planePts[i].y > curMax.y)
                curMax.y = planePts[i].y;
        }
        {
            if (curMin.x < min.x)
                min.x = curMin.x;
        ...
        sizes[idx] = cv::Size((curMax.x - curMin.x) * d->lengthPixelInv(), (curMax.y - curMin.y) * d->lengthPixelInv());
    ...
    for (int x = 0; x < wCopy; x++)
        for (int y = 0; y < hCopy; y++) {
            ...
            if (ele->Ischanged && ticTac.Tac() < 0.02) {
                ...
                glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, ele->img.cols, ele->img.rows, 0, GL_BGRA, GL_UNSIGNED_BYTE,
                        ele->img.data);
