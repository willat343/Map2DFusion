...
class Map2DGPU : public Map2D, public pi::Thread {
    ...
    struct Map2DGPUData  // change when spread and prepare
    {
        Map2DGPUData()
            : _w(0),
              _h(0) {}
        Map2DGPUData(double eleSize_, double lengthPixel_, pi::Point3d max_, pi::Point3d min_, int w_, int h_,
                const std::vector<SPtr<Map2DGPUEle> >& d_)
            : _eleSize(eleSize_),
